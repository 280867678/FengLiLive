...
import com.slzhibo.library.analytics.PropertyBuilder;
import com.slzhibo.library.analytics.SensorsDataAPI;
import com.slzhibo.library.analytics.SensorsDataDynamicSuperProperties;
...
import com.slzhibo.library.utils.live.LiveDataTimer;
...
public class LogEventUtils {
    ...
    public static void uploadSearchResultEvent(String str, String str2, String str3, String str4) {
        if (!intercept(LogConstants.SEARCH_RESULT_CLICK_EVENT_NAME)) {
            SensorsDataAPI.sharedInstance().track(LogConstants.SEARCH_RESULT_CLICK_EVENT_NAME, PropertyBuilder.newInstance().append(LogConstants.ANCHOR_ID, str).append(LogConstants.ANCHOR_APP_ID, str2).append(LogConstants.ANCHOR_NICK_NAME, str3).append("keyWord", str4).toJSONObject());
        }
    }
    ...
    public static void uploadSendSearchRequest(boolean z, boolean z2, boolean z3, String str) {
        if (!intercept(LogConstants.SEND_SEARCH_REQUEST_EVENT_NAME)) {
            ...
            sharedInstance.track(LogConstants.SEND_SEARCH_REQUEST_EVENT_NAME, append.append(LogConstants.IS_RECOMMEND_WORD_USED, str2).append("keyWord", str).toJSONObject());
        }
    }
    ...
    public static void uploadLeaveRoom(LiveDataEntity liveDataEntity) {
        if (!intercept(LogConstants.LEAVE_ROOM_EVENT_NAME) && liveDataEntity != null) {
            long j = ((liveDataEntity.endTime - liveDataEntity.startTime) % DateUtils.ONE_HOUR_MILLIONS) / 60000;
            if (j >= 1) {
    ...
    public static void uploadEndLive(AnchorLiveDataEntity anchorLiveDataEntity) {
        if (!intercept(LogConstants.END_LIVE_EVENT_NAME) && anchorLiveDataEntity != null) {
            long j = ((anchorLiveDataEntity.endTime - anchorLiveDataEntity.startTime) % DateUtils.ONE_HOUR_MILLIONS) / 60000;
            if (j >= 1) {
...
//        if (isReport()) {
//            SensorsDataAPI.sharedInstance().registerDynamicSuperProperties(new SensorsDataDynamicSuperProperties() { // from class: com.slzhibo.library.utils.-$$Lambda$LogEventUtils$mp-S_dPnUOfo6G0T-ZPGzcNV17k
//                @Override // com.slzhibo.library.analytics.SensorsDataDynamicSuperProperties
