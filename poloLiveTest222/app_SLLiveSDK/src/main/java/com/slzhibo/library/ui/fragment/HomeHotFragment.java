...
//        PlayManager playManager = this.playManager;
        LiveEntity liveEntity = (LiveEntity) baseQuickAdapter.getItem(i);
        if(liveEntity == null){
            Toast.makeText(getContext(), TAG+"liveEntity == null", Toast.LENGTH_SHORT).show();
        }
        if(liveEntity.isCoverPreview()){
            Toast.makeText(getContext(), TAG+"liveEntity.isCoverPreview()", Toast.LENGTH_SHORT).show();
        }
        if(playManager  == null){
            Toast.makeText(getContext(), TAG+"playManager  == null", Toast.LENGTH_SHORT).show();
        }
        if (!(liveEntity == null || !liveEntity.isCoverPreview() || playManager  == null)) {
            playManager.playVideoByPosition(1+i + baseQuickAdapter.getHeaderLayoutCount());
        }
        return true;
    }
    ...
    class AnonymousClass2 extends RecyclerView.OnScrollListener {
        ...
        public /* synthetic */ void lambda$onScrollStateChanged$0$HomeHotFragment$2() {
            ...
            if (playManager != null) {
                playManager.onScrolled();
                playManager.onScrollStateChanged();
            }
        }
    }
