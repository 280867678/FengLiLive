package com.slzhibo.library.analytics;

/* loaded from: classes6.dex */
public interface ThreadNameConstants {
    public static final String THREAD_APP_END_DATA_SAVE_TIMER = "SA.AppEndDataSaveTimerThread";
    public static final String THREAD_GET_SDK_REMOTE_CONFIG = "SA.GetSDKRemoteConfigThread";
    public static final String THREAD_SEND_DISTINCT_ID = "SA.SendDistinctIDThread";
    public static final String THREAD_TASK_QUEUE = "SA.TaskQueueThread";
}
